...
// This transformation rule tries to identify the LeakyRelu structure generated by
// Keras, and convert it to a single op.
//
// The formula of LeakyRelu is:
// f(x) = alpha * x for x < 0, f(x) = x for x >= 0.
//
// `x` is the input, and `alpha` is a trainable tensor which can be broadcasted
// to the shape of `x`.
//
// There's no native LeakyRelu op in TensorFlow, so Keras generates the following
...
::tensorflow::Status IdentifyLeakyRelu::Run(Model* model, std::size_t op_index,
                                            bool* modified) {
  *modified = false;
  ...
  leakyrelu_op->inputs = {input_tensor_name, alpha_tensor_name};
  leakyrelu_op->outputs = {output_tensor_name};
  model->operators.emplace(add_op_it, leakyrelu_op);
  AddMessageF("Creating %s replacing equivalent subgraph", LogName(*leakyrelu_op));
